...
<head>
    ...
    function drawColumnChart() {
        ...
        <?php
        $hh = new hanghoa();
        if (isset($_SESSION['nam'],$_SESSION['thang'])) {
            $result = $hh->getThongKeT($_SESSION['nam'], $_SESSION['thang']);
        } else {
            $result = $hh->getThongKeT();
        }
        while ($set = $result->fetch()) {
            $tenhh = $set['tenhh'];
            $slb = $set['soluong'];
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="chart-container">
                    <h2 class='text-center'>Thống kê số lượng bán theo tháng</h2>
        ...
        <div class="row mt-5">
            <div class="col-md-12">
                <div class="form-container mt-5 text-center">
                    <form action="index.php?action=thongkethang&act=thongke_thang" method="post">
                        <h2>Thống kê theo ngày</h2>
                        <select class="form-control w-50 d-inline mt-3" name="thang_nam" id="mySelect">
                            ...
                            while ($set = $dates->fetch()) {
                                $value = $set['nam'] . '-' . str_pad($set['thang'], 2, '0', STR_PAD_LEFT) ;
                                $display = $set['nam'] . '/' . str_pad($set['thang'], 2, '0', STR_PAD_LEFT);
                            ?>
                            <option value="<?php echo $value; ?>" <?php if (isset($_SESSION['thang_nam']) && $value == $_SESSION['thang_nam']) {
                                echo "selected";
                                unset($_SESSION['thang_nam']);
                            } ?>>
                                <?php echo $display; ?>
                            </option>
                            <?php
