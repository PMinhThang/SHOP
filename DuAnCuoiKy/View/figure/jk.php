...
<section id="examples" class="text-center" style="margin-top:-20px">
        ...
        <div class="col-md-6">
            <div class="row">
                <div class="col-md-8 text-right">
                    <u><i>
                            <h3 class="product_name text-left">Jujustu Kaisen</h3>
                        </i></u>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="row">
                <select class="form-control" id="mySelect"
                    ...
                    <option value="index.php?action=phanloai&act=jk">Tất cả</option>
                    <option value="index.php?action=phanloai&act=jk1">Đơn giá tăng dần</option>
                    <option value="index.php?action=phanloai&act=jk2">Đơn giá giảm dần</option>
                    <option value="index.php?action=phanloai&act=jk3">Sắp xếp A - Z</option>
                </select>
            </div>
        </div>
    </div>
    ...
    if (isset($_GET['action'])) {
        if (isset($_GET['act']) && $_GET['act'] == 'jk') {
            $ac = 1;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'jk1') {
            $ac = 2;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'jk2') {
            $ac = 3;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'jk3') {
            $ac = 4;
        }
    }
    ?>
    ...
    <div class="row">
        ...
        if ($ac == 1) {
            ...
            $result = $hh->getAJK($start, $limit);
            // $result=$hh->getHangHoaAll();// lấy được 14 sản phẩm *****
        }
        if ($ac == 2) {
            $gg = new locsanpham();
            $result = $gg->getUJK();// phân trang
        }
        if ($ac == 3) {
            $gg = new locsanpham();
            $result = $gg->getDJK();// phân trang
        }
        if ($ac == 4) {
            $gg = new locsanpham();
            $result = $gg->getJKAZ();// phân trang
        }
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            ...
            if ($current_page > 1 && $page > 1) {
                echo '<li>
                    <a href="index.php?action=phanloai&act=jk&page=' . ($current_page - 1) . '" class="page-link">Trước</a>
                    </li>';
            }
            for ($i = 1; $i <= $page; $i++) {
                ?>
                <li class="page-item <?php echo ($i == $current_page) ? 'active' : ''; ?>">
                    <a href="index.php?action=phanloai&act=jk&page=<?php echo $i; ?>" class="page-link"><?php echo $i; ?></a>
                </li>
                <?php
            }
            if ($current_page < $page && $page > 1) {
                echo '<li>
                    <a href="index.php?action=phanloai&act=jk&page=' . ($current_page + 1) . '" class="page-link">Sau</a>
                    </li>';
            }
            ?>
        </ul>
