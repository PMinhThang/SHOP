...
<section id="examples" class="text-center" style="margin-top:-20px">
    ...
    <div class="col-md-12">
        <div class="col-md-6">
            <div class="row">
                ...
                <u><i>
                        <h3 class="product_name text-left">Attack on Titan</h3>
                    </i></u>
        ...
        <div class="col-md-6">
            <div class="row">
                <select class="form-control" id="mySelect"
                    ...
                    <option value="index.php?action=phanloai&act=aot">Tất cả</option>
                    <option value="index.php?action=phanloai&act=aot1">Đơn giá tăng dần</option>
                    <option value="index.php?action=phanloai&act=aot2">Đơn giá giảm dần</option>
                    <option value="index.php?action=phanloai&act=aot3">Sắp xếp A - Z</option>
                </select>
            </div>
        </div>
    </div>
    ...
    if (isset($_GET['action'])) {
        if (isset($_GET['act']) && $_GET['act'] == 'aot') {
            $ac = 1;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'aot1') {
            $ac = 2;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'aot2') {
            $ac = 3;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'aot3') {
            $ac = 4;
        }
    }
    ?>
    ...
    <div class="row">
        ...
        if ($ac == 1) {
            ...
            $result = $hh->getAAOT($start, $limit);
        }
        if ($ac == 2) {
            $gg = new locsanpham();
            $result = $gg->getUAOT();// phân trang
        }
        if ($ac == 3) {
            $gg = new locsanpham();
            $result = $gg->getDAOT();// phân trang
        }
        if ($ac == 4) {
            $gg = new locsanpham();
            $result = $gg->getAOTAZ();// phân trang
        }
                    ...
                    <a href="index.php?action=sanpham&act=sanphamchitiet&id=<?php echo $set['mahh']; ?>">
                        <div class="product">
                            <div class="view overlay z-depth-1-half">
                                <img style="border-radius: 55px 55px 0 0;width:350px;height:350px;"
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            ...
            if ($current_page > 1 && $page > 1) {
                echo '<li>
                    <a href="index.php?action=phanloai&act=aot&page=' . ($current_page - 1) . '" class="page-link">Trước</a>
                    </li>';
            }
            for ($i = 1; $i <= $page; $i++) {
                ?>
                <li class="page-item <?php echo ($i == $current_page) ? 'active' : ''; ?>">
                    <a href="index.php?action=phanloai&act=aot&page=<?php echo $i; ?>" class="page-link"><?php echo $i; ?></a>
                </li>
                <?php
            }
            if ($current_page < $page && $page > 1) {
                echo '<li>
                    <a href="index.php?action=phanloai&act=aot&page=' . ($current_page + 1) . '" class="page-link">Sau</a>
                    </li>';
            }
            ?>
        </ul>
