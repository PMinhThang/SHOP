...
<section id="examples" class="text-center" style="margin-top:-20px">
        ...
        <div class="col-md-6">
            <div class="row">
                <div class="col-md-8 text-right">
                    <u><i>
                            <h3 class="product_name text-left">Dragon Ball</h3>
                        </i></u>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="row">
                <select class="form-control" id="mySelect"
                    ...
                    <option value="index.php?action=phanloai&act=db">Tất cả</option>
                    <option value="index.php?action=phanloai&act=db1">Đơn giá tăng dần</option>
                    <option value="index.php?action=phanloai&act=db2">Đơn giá giảm dần</option>
                    <option value="index.php?action=phanloai&act=db3">Sắp xếp A - Z</option>
                </select>
            </div>
        </div>
    </div>
    ...
    if (isset($_GET['action'])) {
        if (isset($_GET['act']) && $_GET['act'] == 'db') {
            $ac = 1;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'db1') {
            $ac = 2;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'db2') {
            $ac = 3;
        } elseif (isset($_GET['act']) && $_GET['act'] == 'db3') {
            $ac = 4;
        }
    }
    ?>
    ...
    <div class="row">
        ...
        if ($ac == 1) {
            ...
            $result = $hh->getADB($start, $limit);
        }
        if ($ac == 2) {
            $gg = new locsanpham();
            $result = $gg->getUDB();// phân trang
        }
        if ($ac == 3) {
            $gg = new locsanpham();
            $result = $gg->getDDB();// phân trang
        }
        if ($ac == 4) {
            $gg = new locsanpham();
            $result = $gg->getDBAZ();// phân trang
        }
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            ...
            if ($current_page > 1 && $page > 1) {
                echo '<li>
                    <a href="index.php?action=phanloai&act=db&page=' . ($current_page - 1) . '" class="page-link">Trước</a>
                    </li>';
            }
            for ($i = 1; $i <= $page; $i++) {
                ?>
                <li class="page-item <?php echo ($i == $current_page) ? 'active' : ''; ?>">
                    <a href="index.php?action=phanloai&act=db&page=<?php echo $i; ?>" class="page-link"><?php echo $i; ?></a>
                </li>
                <?php
            }
            if ($current_page < $page && $page > 1) {
                echo '<li>
                    <a href="index.php?action=phanloai&act=db&page=' . ($current_page + 1) . '" class="page-link">Sau</a>
                    </li>';
            }
            ?>
        </ul>
